...
const TodoList = ({ onTaskComplete }) => {
    ...
    const handleAddTodo = () => {
        if (newTodo.trim() !== "") {
            const newTodoItem = { id: Date.now(), text: newTodo, completed: false, createdAt: getTodayDateString() };
    ...
    const handleStartEdit = (todo) => { setEditingTodo(todo.id); setEditingText(todo.text); };
    const handleSaveEdit = (id) => { setTodos(todos.map(todo => todo.id === id ? { ...todo, text: editingText } : todo)); setEditingTodo(null); setEditingText(""); };
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-sm">
            ...
            <div className="flex gap-2 mb-4">
                ...
                />
                <button onClick={handleAddTodo} className="px-4 py-2 bg-cyan-500 text-white font-semibold rounded-md hover:bg-cyan-600">추가</button>
            </div>
            <div className="mt-6">
                {sortedDates.map(date => (
                    <div key={date} className="mb-6">
                        <h3 className="font-bold border-b pb-2 mb-3">{new Date(date).toLocaleDateString('ko-KR', { year: 'numeric', month: 'long', day: 'numeric', weekday: 'long' })}</h3>
                        <ul className="space-y-3">
                            {groupedTodos[date].map(todo => (
                                <li key={todo.id} className={`flex items-center p-3 bg-gray-50 rounded-md transition-all hover:bg-gray-100 ${todo.id === justCompletedId ? 'animate-tada' : ''}`}>
                                    <button onClick={() => handleToggleComplete(todo.id)} className={`mr-3 ${todo.completed ? 'text-cyan-500' : 'text-gray-300'}`}><CheckIcon /></button>
                                    <div className="flex-1">
                                        {editingTodo === todo.id ? (
                                            <input
                                                type="text" value={editingText} onChange={(e) => setEditingText(e.target.value)}
                                                onKeyPress={(e) => e.key === 'Enter' && handleSaveEdit(todo.id)}
                                                onBlur={() => handleSaveEdit(todo.id)} autoFocus
